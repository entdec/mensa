...
class UsersTablesTest < ApplicationSystemTestCase
  ...
  test "sorting by last name orders rows lexicographically" do
    ...
    assert_equal last_names, last_names.sort_by(&:downcase)
  end
  ...
  test "clicking the remove button on a filter pill clears the filter" do
    ...
    assert_no_selector ".mensa-filter-pill", wait: 10
    # All 20 first-page rows should be back
    assert_selector "tbody tr", minimum: 20, wait: 10
  end
  ...
  test "a role filter combined with a text search narrows results further" do
    ...
    assert_selector "tbody tr", text: "Oliver", wait: 15
  ...
  test "editing a filter pill and selecting a new value updates the table" do
    ...
    find(".mensa-filter-pill__chip").click
    find("[data-mensa-add-filter-target='valueOption'][data-value='guest']", wait: 10).click

    assert_selector ".mensa-filter-pill__value", text: "guest", wait: 10
  ...
  test "changing the filter operator to 'not is' inverts the filtered result set" do
    ...
    assert_selector "tbody tr", minimum: 2, wait: 10
  ...
  test "hiding a column via the column customizer removes it from the table header" do
    visit users_url
    assert_selector "thead th", text: "Role", wait: 15

    open_column_customizer
    toggle_column_visibility("role")

    assert_no_selector "thead th", text: "Role", wait: 10
  end
  ...
  test "toggling a hidden column visible restores it in the table header" do
    ...
    # Toggle it back on
    toggle_column_visibility("role")
    assert_selector "thead th", text: "Role", wait: 10
  end
  ...
  test "column visibility state survives a page reload via localStorage" do
    ...
    visit users_url
    assert_no_selector "thead th", text: "Role", wait: 15
  end
  ...
  test "dragging a column in the customizer reorders it in the table header" do
    ...
    assert_selector "thead th", wait: 15

    headers = all("thead th").filter_map { |th| th.text.strip.presence }
  ...
  test "saving filters as a new view adds it to the views dropdown" do
    ...
    find("[data-mensa-table-target='saveSimple']", wait: 10).click
  ...
  test "switching away from a custom view and back restores its filters" do
    ...
    add_filter(column: "role", value: "admin")
    assert_selector "[data-mensa-table-target='saveSimple']:not(.hidden)", wait: 10
    ...
    assert_selector ".mensa-table__views__trigger-label", text: "All users", wait: 10
    ...
    assert_selector ".mensa-table__views__trigger-label", text: "Admins only", wait: 10
    assert_selector ".mensa-filter-pill__value", text: "admin", wait: 10
  end
  ...
  test "renaming a view updates its name in the dropdown and trigger label" do
    ...
    assert_selector ".mensa-table__views__trigger-label", text: "Before rename", wait: 10

    # Open the ellipsis submenu and rename
    open_views_dropdown
    find("li[data-view-id='#{view.id}'] .mensa-table__views__option-menu").click
    find("[data-action='mensa-views#renameView']", wait: 10).click
    ...
    assert_selector ".mensa-table__views__trigger-label", text: "After rename", wait: 10
    open_views_dropdown
  ...
  test "deleting a view removes it from the dropdown" do
    ...
    find("li[data-view-id='#{view.id}'] .mensa-table__views__option-menu").click
    accept_confirm do
      find("[data-action='mensa-views#deleteView']", wait: 10).click
    end

    open_views_dropdown
    assert_no_selector "li[data-view-id='#{view.id}']", wait: 10
  end
  ...
  test "after deleting the active view the dropdown reverts to All users" do
    ...
    open_views_dropdown
    find("li[data-view-id='#{view.id}'] button[data-action='mensa-views#select']").click
    assert_selector ".mensa-table__views__trigger-label", text: "To be deleted", wait: 10
  ...
  test "switching to the Users built-in view hides the Role column" do
    ...
    assert_selector ".mensa-table__views__trigger-label", text: "Users", wait: 10
    assert_no_selector "thead th", text: "Role", wait: 10
  end
  ...
  test "the active view check indicator updates when switching views" do
    ...
    assert_selector "li[data-view-id='users'] .mensa-table__views__option-check.invisible"

    find("li[data-view-id='users'] button[data-action='mensa-views#select']").click
    open_views_dropdown

    assert_selector "li[data-view-id='users'] .mensa-table__views__option-check:not(.invisible)",
                    wait: 10
    assert_selector "li[data-view-id='default'] .mensa-table__views__option-check.invisible"
  end
  ...
  def add_filter(column:, value:)
    find(".mensa-table__add_filter__trigger").click
    find("[data-filter-column-name='#{column}']", wait: 10).click
    find("[data-mensa-add-filter-target='valueOption'][data-value='#{value}']", wait: 10).click
  ...
  def open_column_customizer
    ...
    assert_selector ".mensa-table__column_customizer__popover:not(.hidden)", wait: 10
  end
